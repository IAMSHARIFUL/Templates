...
int main()
{
    ...
    for(j=0; j<3; j++)
    {
        ...
        for(i=0; i<n; i++)
        {
            ...
            cities[j].push_back(p);
        }
    }
    D=Minkowski::minkowski(cities[0],cities[1]);
    D=Minkowski::minkowski(D,cities[2]);
    for(i=0; i<D.size(); i++)
    {
        if(D[i].x<trans.x||(D[i].x==trans.x&&D[i].y<trans.y))
        {
            trans.x=D[i].x;
            trans.y=D[i].y;
        }
    }
    pointLocationInPolygon plp(D);
    scanf("%d",&q);
    for(i=0; i<q; i++)
    {
        scanf("%I64d %I64d",&p.x,&p.y);
